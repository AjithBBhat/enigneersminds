...
function EditPost({ page = "admin" }) {
  ...
  const handlePublish = async () => {
    try {
     ...
     // Add featuredImage property only if it's not undefined
     if (media?.selected?._id) {
      postData.featuredImage = media.selected._id;
  } else if (featuredImage?._id) {
      postData.featuredImage = featuredImage._id;
  }

  // Make the PUT request with the constructed data object
